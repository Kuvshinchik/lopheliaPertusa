...
class AdminTableUpdateController extends Controller
{ 
    protected $alias;
	protected $request;
	...
	private function  massivTable(): void {
		...
		if(isset($_POST)){
			//dd($_POST);
			//нужно определьть номер элемента с id, поскольку с него начинается рабочая информация
			//все что выше, то служебные элементы и не нужны
			//также нужно исключить из обработки элементы, которые обрабатываются в БД автоматически
			$startElement = array_search('id',array_keys($_POST));
			//dd($startElement);
			$massivForBD_01 = [];
			$i = 0;
			foreach($_POST as $key=>$value){				
				if (($i < $startElement) || (in_array($key, ['id', 'created_at', 'updated_at', 'email_verified_at', 'ДЕЙСТВИЕ', 'file'])))
				{
					$i++;
					continue;
				}else{
